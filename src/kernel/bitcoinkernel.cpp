        ...
        bool blockfiles_only)
{
    ...
    LOCK(opts->m_mutex);
    opts->blockfiles_only = blockfiles_only;
}
    ...
    const kernel_ChainstateManagerOptions* chainman_opts_)
{
    ...
    try {
        ...
        if (chainman_opts->blockfiles_only) {
            chainman->SetBlockfilesOnly(true);
        }
    } catch (const std::exception& e) {
    ...
    try {
        ...
        for (Chainstate* chainstate : WITH_LOCK(chainman->GetMutex(), return chainman->GetAll())) {
            ...
            if (chainman->IsBlockfilesOnly()) {
                chainstate->SetBlockfilesOnly(true);
            }
