...
public:
    ...
    {
    private:
        std::vector<CBlockIndex*> m_base;
        std::vector<CBlockIndex*> m_tail;

        friend class CChain;
        Snapshot(std::vector<CBlockIndex*> base,
    ...
    public:
        ...
        {
            if (!m_base.empty()) return m_base[0];
            if (!m_tail.empty()) return m_tail[0];
            return nullptr;
        }
        ...
        {
            if (!m_tail.empty()) return m_tail.back();
            if (!m_base.empty()) return m_base.back();
            return nullptr;
        }
        ...
        int Height() const
        {
            return int(m_base.size() + m_tail.size()) - 1;
        }
        ...
        bool empty() const
        {
            return m_base.empty() && m_tail.empty();
        }
    };

private:
    mutable Mutex m_write_mutex;
    stlab::copy_on_write<Snapshot> m_snapshot;
    ...
    {
        auto& writable_snapshot = m_snapshot.write();

        std::vector<CBlockIndex*> new_base;
        new_base.reserve(new_base.size() + writable_snapshot.m_tail.size() + 1);
        new_base.insert(new_base.end(), writable_snapshot.m_base.begin(), writable_snapshot.m_base.end());
        new_base.insert(new_base.end(), writable_snapshot.m_tail.begin(), writable_snapshot.m_tail.end());
        new_base.push_back(&block);

        writable_snapshot.m_base = std::move(new_base);
        writable_snapshot.m_tail.clear();
    }

    void AppendToTail(CBlockIndex& block)
    {
        auto& writable_snapshot = m_snapshot.write();
        writable_snapshot.m_tail.push_back(&block);
    }
