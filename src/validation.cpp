    ...
    bool fAddToMempool)
{
    ...
    int tip_height = snapshot.Height();
    ...
    const auto filter_final_and_mature = [&, tip, tip_height](CTxMemPool::txiter it)
        ...
        if (!CheckFinalTxAtTip(*Assert(tip), tx)) return true;

        const LockPoints& lp = it->GetLockPoints();
        // CheckSequenceLocksAtTip checks if the transaction will be final in the next block to be
        // created on top of the new chain.
        if (TestLockPointValidity(m_chain, lp)) {
            if (!CheckSequenceLocksAtTip(tip, lp)) {
                return true;
            }
        } else {
            const CCoinsViewMemPool view_mempool{&CoinsTip(), *m_mempool};
            const std::optional<LockPoints> new_lock_points{CalculateLockPointsAtTip(tip, view_mempool, tx)};
            if (new_lock_points.has_value() && CheckSequenceLocksAtTip(tip, *new_lock_points)) {
        ...
        if (it->GetSpendsCoinbase()) {
            for (const CTxIn& txin : tx.vin) {
                ...
                assert(!coin.IsSpent());
                const auto mempool_spend_height{tip_height + 1};
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    CChain snapshot = m_active_chainstate.m_chain;
    CBlockIndex* tip = snapshot.Tip();
    int tip_height = snapshot.Height();
    ...
    // be mined yet.
    if (!CheckFinalTxAtTip(*Assert(tip), tx)) {
    ...
    const std::optional<LockPoints> lock_points{CalculateLockPointsAtTip(tip, m_view, tx)};
    if (!lock_points.has_value() || !CheckSequenceLocksAtTip(tip, *lock_points)) {
        return state.Invalid(TxValidationResult::TX_PREMATURE_SPEND, "non-BIP68-final");
    }

    // The mempool holds txs for the next block, so pass height+1 to CheckTxInputs
    if (!Consensus::CheckTxInputs(tx, state, m_view, tip_height + 1, ws.m_base_fees)) {
    ...
    ws.m_tx_handle = m_subpackage.m_changeset->StageAddition(ptx, ws.m_base_fees, nAcceptTime, tip_height, entry_sequence, fSpendsCoinbase, nSigOpsCost, lock_points.value());
...
bool Chainstate::PreciousBlock(BlockValidationState& state, CBlockIndex* pindex)
{
    ...
    AssertLockNotHeld(::cs_main);
    {
        LOCK(cs_main);
        CChain snapshot = m_chain;
        if (pindex->nChainWork < snapshot.Tip()->nChainWork) {
            // Nothing to do, this block is not at the tip.
            return true;
        }
        if (snapshot.Tip()->nChainWork > m_chainman.nLastPreciousChainwork) {
            // The chain has been extended since the last call, reset the counter.
            m_chainman.nBlockReverseSequenceId = -1;
        }
        m_chainman.nLastPreciousChainwork = snapshot.Tip()->nChainWork;
        setBlockIndexCandidates.erase(pindex);
