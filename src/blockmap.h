...
public:
    ...
    private:
        mutable std::pair<uint256, CBlockIndex*> m_cached_value_storage;

    public:
        iterator() : m_in_stable(false) {}
        ...
        iterator(InnerMap::const_iterator stable_begin, InnerMap::const_iterator stable_end,
                 ...
                 bool at_end = false)
            : m_stable_it(stable_begin), m_stable_end(stable_end), m_recent_it(recent_begin), m_recent_end(recent_end), m_in_stable(!at_end && stable_begin != stable_end)
        {
            if (at_end) {
                m_in_stable = false;
        ...
        {
            if (m_in_stable) {
                return {m_stable_it->first, m_stable_it->second.get()};
            } else {
                return {m_recent_it->first, m_recent_it->second.get()};
            }
        }
        ...
        {
            if (m_in_stable) {
                m_cached_value_storage = {m_stable_it->first, m_stable_it->second.get()};
            } else {
                m_cached_value_storage = {m_recent_it->first, m_recent_it->second.get()};
            }

            return reinterpret_cast<pointer>(&m_cached_value_storage);
        }
    ...
    public:
        const_iterator() : m_in_stable(false) {}
        ...
        const_iterator(const iterator& other)
            : m_stable_it(other.m_stable_it), m_stable_end(other.m_stable_end), m_recent_it(other.m_recent_it), m_recent_end(other.m_recent_end), m_in_stable(other.m_in_stable)
        {
        }
