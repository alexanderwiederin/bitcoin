...
public:
    ...
    public:
        ...
        iterator(InnerMap::const_iterator stable_begin, InnerMap::const_iterator stable_end,
                 ...
                 bool at_end = false)
            : m_stable_it(stable_begin), m_stable_end(stable_end),
            m_recent_it(recent_begin), m_recent_end(recent_end),
            m_in_stable(!at_end && stable_begin != stable_end),
            m_cached_value({uint256(), nullptr})
        {
            if (at_end) {
                m_in_stable = false;
